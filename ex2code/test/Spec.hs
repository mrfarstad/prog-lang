...
main =
  hspec $ do
    ...
    describe "map" $ do
      it "returns an empty list for f []" $ do L.map (+ 1) [] `shouldBe` []
    ...
    describe "filterPos" $ do
      it "returns empty list for empty list" $ do filterPos [] `shouldBe` []
    ...
    describe "isPerfSq" $ do
      it "werkz" $ do isPerfSq 16 `shouldBe` True
      it "werkz" $ do isPerfSq 5 `shouldBe` False
